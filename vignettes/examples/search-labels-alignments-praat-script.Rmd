...
sptkV.matches <- getMatches(labbcat.url, list(syllables = sptkV, segments = "s"), no.progress = T)
sptkr.matches <- getMatches(labbcat.url, list(syllables = sptkr, segments = "s"), no.progress = T)
stj.matches <- getMatches(labbcat.url, list(syllables = stj, segments = "s"), no.progress = T)

print(c(paste("There are", nrow(sptkr.matches), "tokens of ...s[ptk]r..."),
...
```{r comparison}
sSV.matches <- getMatches(labbcat.url, list(phonemes = sSV, segments = "[sS]"), no.progress = T)

print(paste("There are", nrow(sSV.matches), "reference tokens of [sS]V..."))
```
...
print(paste("Total tokens:", nrow(matches)))
```
...
print(head(matches)[, c(
  "Text", "Target.segments", "Target.segments.start", "Target.segments.end")])
```
...
print(head(matches)[, c(
  "Text", "Target.segments", "syllables", "syllables.start", "syllables.end")])
```

And the start/end times for the segment that *follows* the token:

```{r following-segment}
following.segment <- getMatchAlignments(labbcat.url, matches$MatchId, c("segments"), targetOffset = 1)

matches <- cbind(matches, following.segment)
print(head(matches)[, c(
  "Token.plus.1.segments", "Token.plus.1.segments.start", "Token.plus.1.segments.end")])
```
...
cog <- processWithPraat(
  labbcat.url,
  matches$MatchId, matches$Target.segments.start, matches$Target.segments.end,
  script, window.offset=0.5, 
  no.progress = T)

matches <- cbind(matches, cog)
print(head(matches)[, c("Text", "cog1", "cog2", "cog3")])
```
