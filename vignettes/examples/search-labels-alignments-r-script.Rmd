...
print(head(matches)[, c("Transcript", "Target.transcript", "Target.transcript.start")])
```
...
segments <- getMatchAlignments(
  labbcat.url, matches$MatchId, c("segments"), annotationsPerLayer=3)

matches <- cbind(matches, segments)
print(head(matches)[, c(
  "Text", "segments.3", "segments.3.start", "segments.3.end")])
```
